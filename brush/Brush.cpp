...
int Brush::getIndex(int row, int col, int col_number) {
    return (row * col_number + col);
}


void Brush::paint(int x, int y, int width, int height, RGBA* pix, RGBA color, bool alphaBlend){
...
RGBA Brush::alphaBlending(RGBA new_Color, RGBA old_Color, float m) {


    float a = new_Color.a / 255.f;
...
float Brush::dist(int x1, int y1, int x2, int y2, int r) {


    return ( std::sqrt( std::pow( ((x2-1)-x1) , 2) + std::pow( ((y2-1)-y1), 2)));
}
